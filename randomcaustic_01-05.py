...
def Scan(begin, end, freq):
    ...
    res = np.linspace(-1/2,1/2,5,endpoint='true')[:-1]
    lensed = []
    spec = []
    for i in scan:
...
dispath = dispath[2800-1200:2800+1200]
...
for i in range(1,6):
    ...
    np.save(FileName + format(i, '02') + '-' + format(freq/10**6, '.2f') + "Mag"+format(scan, '04') + "to" + format(scan+diff, '04'),mag)
    np.save(FileName + format(i, '02') + '-' + format(freq/10**6, '.2f') + "Spec"+format(scan, '04') + "to" + format(scan+diff, '04'),spec)
