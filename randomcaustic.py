...
for i in range(12):
    dispath = np.load('data/causticDis'+format(i, '01')+'.npy')

    mag, spec = Scan(scan,scan+diff,freq)
    np.save(FileName + format(i, '01') + '-' + format(freq/10**6, '.2f') + "Mag"+format(scan, '04') + "to" + format(scan+diff, '04'),mag)
    np.save(FileName + format(i, '01') + '-' + format(freq/10**6, '.2f') + "Spec"+format(scan, '04') + "to" + format(scan+diff, '04'),spec)
